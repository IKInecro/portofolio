    ...
    /* ===== PORTO DESIGN MODAL — brutalism — fix klik tidak keluar (samain gallery) ===== */
    let portoIndex = 0;
    function getPortoItems() {
      return Array.from(document.querySelectorAll('#posterGrid .poster-item')).map(el => {
        const m = el.getAttribute('onclick') || '';
        const srcMatch = m.match(/'([^']+\.webp)'/);
        const titleMatch = m.match(/,\s*'([^']+)'/);
        return { src: srcMatch ? srcMatch[1] : '', title: titleMatch ? titleMatch[1] : '' };
      }).filter(p => p.src);
    }
    function openPortoModal(src, title) {
      const items = getPortoItems();
      const idx = items.findIndex(p => p.src === src);
      portoIndex = idx >= 0 ? idx : 0;
      window._portoItems = items;
      const modal = document.getElementById('portoModal');
      const img = document.getElementById('portoImage');
      if (img) img.src = src;
      const cap = document.getElementById('portoCaption');
      if (cap) cap.textContent = title;
      const dl = document.getElementById('portoDownload');
      if (dl) dl.href = src;
      const counter = document.getElementById('portoCounter');
      if (counter) counter.textContent = (portoIndex + 1) + ' / ' + items.length;
      if (modal) modal.classList.add('active');
      document.body.style.overflow = 'hidden';
    }
    window.openPortoModal = openPortoModal;
    function closePortoModal() {
      const modal = document.getElementById('portoModal');
      if (modal) modal.classList.remove('active');
      document.body.style.overflow = '';
    }
    window.closePortoModal = closePortoModal;
    window.portoPrev = function() {
      const items = window._portoItems || getPortoItems();
      portoIndex = (portoIndex - 1 + items.length) % items.length;
      const p = items[portoIndex];
      document.getElementById('portoImage').src = p.src;
      document.getElementById('portoCaption').textContent = p.title;
      document.getElementById('portoDownload').href = p.src;
      document.getElementById('portoCounter').textContent = (portoIndex + 1) + ' / ' + items.length;
    };
    window.portoNext = function() {
      const items = window._portoItems || getPortoItems();
      portoIndex = (portoIndex + 1) % items.length;
      const p = items[portoIndex];
      document.getElementById('portoImage').src = p.src;
      document.getElementById('portoCaption').textContent = p.title;
      document.getElementById('portoDownload').href = p.src;
    ...
    document.addEventListener('keydown', function(e) {
      ...
      if (!modal || !modal.classList.contains('active')) {
        if (e.key === 'Escape') closePortoModal();
        return;
      }
      if (e.key === 'Escape') closePortoModal();
      if (e.key === 'ArrowLeft') portoPrev();
      if (e.key === 'ArrowRight') portoNext();
    });
    // mobile swipe prev/next
    (function() {
      const box = document.getElementById('portoBox');
      if (!box) return;
      let startX = 0;
      ...
      box.addEventListener('touchend', (e) => {
        ...
        if (Math.abs(dx) > 50) {
          if (dx > 0) portoPrev(); else portoNext();
        }
      }, { passive: true });
    })();
